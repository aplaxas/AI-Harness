...
> 이 글은 **왜 harness가 필요하고 그것이 무엇인지**까지를 다룹니다.
> 실제로 Agent와 Skill을 어떻게 쓰는지는 다음 글에서 이어집니다.
...
>
> **3~5문장으로 짧게.** 바로 뒤 1장이 무겁기 때문에, 여기까지 길어지면 도입이 두 번이 됩니다.
...
2025년 2월, 안드레이 카파시(Andrej Karpathy)가 **vibe coding**이라는 말을 만들었습니다.[^karpathy] 코드의 존재 자체를 잊고 흐름에 몸을 맡기라는 것. 자연어로 말하면 코드가 나오는 시대에, 그건 과장이 아니라 실제로 가능해지고 있었습니다.

이건 **선언**입니다. 무엇이 가능해졌는지에 대한.

### 토발즈: 커밋

한편 리누스 토발즈는 선언 대신 커밋을 남겼습니다.[^torvalds]
...
여기서 "그럼 이것도 AI인가?"를 따질 필요는 없습니다. 이름은 중요하지 않습니다. **중요한 건 규칙을 누가 만들었는가입니다.**
...
`"휴스턴은"` 다음에 무엇이 올까요? 한국인이라면 *"텍사스에 있다"*, *"NASA가 있다"*를 떠올릴 겁니다. 정작 휴스턴에 사는 사람은 *"아주 덥다"*를 먼저 떠올립니다.

같은 입력인데 다음 후보의 확률이 다릅니다. 무엇을 많이 봤는지가 다르기 때문입니다.
...
이 네 가지는 5장에서 다시 만납니다. 각각을 어떤 원칙으로 막을 것인지가 harness 설계의 실질적인 내용이기 때문입니다.
...
그리고 이 판단력은 **모델을 바꿔서 얻는 게 아니라, 환경을 설계해서 얻습니다.**
...
1장의 토발즈를 이 표에 얹어보면 이렇게 읽힙니다. `antigravity` 브랜치는 **권한**이고, *"내가 손으로 짠 것보다 나은가"*는 **검증**이며, 커밋 메시지에 남긴 시행착오 기록은 **관측**입니다. 도구를 하나도 새로 만들지 않고도 다섯 축 중 셋이 이미 작동하고 있었습니다.

### 검증은 사람이 아니라 루프가 한다

다섯 축 중 하나만 고른다면 **검증**입니다. AI가 만든 코드를 신뢰하는 유일한 방법은 **믿지 않는 것**이기 때문입니다. 대신 검증합니다.

여기서 핵심은 **검증 결과가 AI에게 자동으로 되먹임되어야 한다**는 점입니다. 사람이 테스트를 돌려보고 결과를 복사해서 알려주는 구조라면, 그건 harness가 아니라 그냥 수동 작업입니다.

이 되먹임이 붙는 순간 비로소 루프가 됩니다. **생성 → 검증 → 수정 → 재검증.** 이 루프 안에서 AI의 비결정성은 결함이 아니라 다시 시도할 수 있는 여지가 됩니다.

### 세 가지 관심사의 분리

> 누가(**Agent**) / 무엇을(**Task**) / 어떻게(**Skill**)를
> **독립적으로 설계하되, 실행 시점에 연동하라.**[^p58]

이 원칙이 harness 설계의 뼈대입니다. 셋을 뭉뚱그리면 — 예를 들어 "리뷰 잘하는 프롬프트" 하나에 역할·작업·방법을 다 우겨넣으면 — 재사용이 안 되고, 어디가 잘못됐는지 좁힐 수도 없습니다.

셋 중 **Task는 매번 달라집니다.** 오늘은 마이그레이션이고 내일은 버그 수정입니다. 그래서 미리 설계할 수 있는 건 **Agent와 Skill**뿐이고, 이 둘을 어떻게 파일로 만드는지가 다음 글의 주제입니다.

---

## 5. 무엇을 강제할 것인가 — Coding Principles

harness가 무엇인지 알았다면 남은 질문은 하나입니다. **그 안에서 무엇을 강제할 것인가?**
...
2장에서 예고한 네 가지 실패 양상이 여기서 각각 짝을 만납니다. **과잉 구현**은 YAGNI와 Rule of Three가, **누락**은 검증 게이트가, **왜곡**은 명시적 용어집이 막습니다. 남은 하나인 **할루시네이션**만은 원칙으로 막을 수 없습니다. 실행해봐야 알 수 있기 때문에, 오직 검증 루프만이 답입니다.
...
harness를 잘 만들어도 AI의 출력은 **확률적 예측**입니다. 전례 없는 상황에서 진짜 실천지혜를 발휘하지는 못합니다.
...
## 다음 글 — 실제로 harness 만들기

여기까지가 설계입니다. 다음 글에서는 실물을 다룹니다.

- **Agent** — 역할을 어떻게 나누고, 권한과 도구를 어디까지 줄 것인가
- **Skill** — 절차를 어떻게 재사용 가능한 형태로 떼어낼 것인가
- **검증 루프** — 어떤 게이트를, 어떤 순서로, 어떤 명령으로 걸 것인가

> ✍️ **[채우기]** 다음 글 파일명이 정해지면 여기에 링크를 거세요.
...
[^torvalds]: ✍️ **[채우기]** 저장소 주소와 커밋 해시를 확인해서 넣으세요.
[^p58]: ✍️ **[채우기]** 원본 메모의 `p58`, `32 Page` 출처를 확인해서 넣으세요. (예: Claude Code 공식 문서, "Agents and Skills", p.58)
